...
def preprocess_input(image, net_h, net_w):
    image = image[..., ::-1]  # BGR to RGB if needed
    new_h, new_w, _ = image.shape

    # Calculate scale and new size
    scale = min(net_w / new_w, net_h / new_h)
    new_w_resized = int(new_w * scale)
    new_h_resized = int(new_h * scale)
    resized = cv2.resize(image, (new_w_resized, new_h_resized))

    # Create new image and paste resized image into center
    new_image = np.full((net_h, net_w, 3), 128, dtype='uint8')
    y_offset = (net_h - new_h_resized) // 2
    x_offset = (net_w - new_w_resized) // 2

    # Use the actual shape of the resized image for slicing
    new_image[y_offset:y_offset+new_h_resized, x_offset:x_offset+new_w_resized, :] = resized
...
#                 cv2.waitKey(5)
#                 cv2.imwrite("C:/Users/padil/OneDrive/Desktop/Traffic-Violation-Detection/IdentifiedPictures/violation_"+str(dcnt)+".jpg", cimg)
#                 dcnt = dcnt+1
#             else:
...
def draw_boxes(image, boxes, line, labels, obj_thresh, dcnt):
    ...
    helmet_boxes = detect_helmets_with_dnn(image)
